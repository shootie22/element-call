...
export interface HostBridge {
  /** Fork-specific call panel integration. */
  notifyDeafen?(state: { deafened: boolean }): Promise<void>;
  deafen$?: Observable<
    HostRequest<{ deafened?: boolean }, { deafened: boolean }>
  >;
  feedOnly$?: Observable<
    HostRequest<{ enabled?: boolean; includeSelf?: boolean }>
  >;
  expand?(): Promise<void>;
  notifyCallMediaState?(state: {
    participants: {
      userId: string;
      deviceId: string;
      displayName: string;
...
export function createWidgetHostBridge(widget: WidgetHelpers): HostBridge {
  ...
  return {
    ...
    deafen$: requests(ElementWidgetActions.Deafen),
    feedOnly$: requests(ElementWidgetActions.FeedOnly),
    notifyDeafen: async (state) => send(ElementWidgetActions.Deafen, state),
    notifyCallMediaState: async (state) =>
      send(ElementWidgetActions.CallMediaState, state),
    expand: async () => send(ElementWidgetActions.Expand),
