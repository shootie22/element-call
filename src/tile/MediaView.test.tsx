...
describe("MediaView", () => {
  ...
  test("disabled feeds unmount video and re-enabling a stopped feed never spins", () => {
    const { rerender } = render(<MediaView {...baseProps} feedDisabled />);
    expect(screen.queryByTestId("video")).not.toBeInTheDocument();
    expect(screen.getByText("Feed disabled")).toBeVisible();
    rerender(<MediaView {...baseProps} videoEnabled={false} />);
    expect(screen.queryByRole("status")).not.toBeInTheDocument();
    expect(screen.queryByText("Feed disabled")).not.toBeInTheDocument();
  });
