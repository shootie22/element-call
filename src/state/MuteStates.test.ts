...
import { muteAllAudio } from "../settings/settings";
...
describe("MuteStates", () => {
  ...
  test.each([true, false])(
    "deafen restores the microphone state (%s) and host queries are read-only",
    async (audioEnabled) => {
      muteAllAudio.setValue(false);
      const deafen$ = new Subject<
        HostRequest<{ deafened?: boolean }, { deafened: boolean }>
      >();
      const notifyDeafen = vi.fn(async () => {});
      const states = new MuteStates(
        testScope,
        mockMediaDevices({
          audioInput: aAudioInput(),
          videoInput: aVideoInput(),
        }),
        { audioEnabled, videoEnabled: false },
        { ...nullHostBridge, deafen$, notifyDeafen },
      );
      await flushPromises();
      states.toggleDeafen();
      await flushPromises();
      expect(states.audio.enabled$.value).toBe(false);
      expect(muteAllAudio.getValue()).toBe(true);
      expect(notifyDeafen).toHaveBeenLastCalledWith({ deafened: true });

      const reply = vi.fn();
      deafen$.next({ data: {}, reply });
      expect(reply).toHaveBeenCalledWith({ deafened: true });
      expect(muteAllAudio.getValue()).toBe(true);
      // Repeated host requests must not overwrite the saved microphone state.
      deafen$.next({ data: { deafened: true }, reply });
      deafen$.next({ data: { deafened: false }, reply });
      await flushPromises();
      expect(states.audio.enabled$.value).toBe(audioEnabled);
      expect(muteAllAudio.getValue()).toBe(false);
      expect(reply).toHaveBeenLastCalledWith({ deafened: false });
    },
  );
