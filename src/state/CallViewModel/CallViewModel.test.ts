...
describe.each(modes)("CallViewModel (%s mode)", (mode) => {
  ...
  test("multiple shares stay selectable in grid and focused shares clear when stopped", () => {
    withTestScheduler(({ behavior, schedule }) => {
      withCallViewModel(
        {
          remoteParticipants$: constant([aliceParticipant, bobParticipant]),
          rtcMembers$: constant([localRtcMember, aliceRtcMember, bobRtcMember]),
          sharingScreen: new Map([
            [aliceParticipant, behavior("y-----n", yesNo)],
            [bobParticipant, constant(true)],
          ]),
        },
        (vm) => {
          const aliceShare = `${aliceId}:0:screen-share`;
          const bobShare = `${bobId}:0:screen-share`;
          schedule("-g-fgf-n", {
            g: () => {
              ...
              if (layout.type === "grid") {
                expect(
                  [...layout.grid].map((tile) => tile.media$.value.id),
                ).toEqual(expect.arrayContaining([aliceShare, bobShare]));
              }
            },
            f: () => {
              vm.focusMedia(aliceShare);
              vm.setGridMode("spotlight");
              const layout = vm.layout$.value;
              expect(layout.type).toBe("spotlight-landscape");
              if (layout.type === "spotlight-landscape") {
                expect(
                  layout.spotlight.media$.value.map((media) => media.id),
                ).toEqual([aliceShare]);
                expect(
                  [...layout.grid].map((tile) => tile.media$.value.id),
                ).toContain(bobShare);
