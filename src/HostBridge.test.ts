...
describe("createWidgetHostBridge", () => {
  describe("telling the host what Element Call is doing", () => {
    ...
    test("preserves the custom call panel message contract", async () => {
      const transport = mockTransport();
      const bridge = createWidgetHostBridge(mockWidget({ transport } as never));
      await bridge.notifyDeafen!({ deafened: true });
      await bridge.notifyCallMediaState!({ participants: [], anyVideo: false });
      await bridge.expand!();
      expect(transport.send).toHaveBeenCalledWith(ElementWidgetActions.Deafen, {
        deafened: true,
      });
      expect(transport.send).toHaveBeenCalledWith(
        ElementWidgetActions.CallMediaState,
        { participants: [], anyVideo: false },
      );
      expect(transport.send).toHaveBeenCalledWith(
        ElementWidgetActions.Expand,
        {},
      );
    });
